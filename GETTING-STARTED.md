...
This is the moment everything goes live. Run:
...
> **Why we no longer pass keys here.** Earlier versions of this guide bundled keys into the deploy command via `--context tenantKeyMap=...` and `--context readKeyMap=...`. That approach was unsafe: forgetting either flag on a redeploy caused CloudFormation to wipe the live keys and replace them with auto-generated rubbish. Keys are now populated **once, manually** in the AWS Console (Step 10) — and every future redeploy leaves them untouched.

---

## Step 10 — Populate your keys (one-time, after first deploy)

After the deploy finishes, the two key vaults exist but are filled with auto-generated placeholder values. Open them in the AWS Console and replace the placeholders with your real key map.

### TenantKeyMapSecret — for customers sending records
...
3. Click **Retrieve secret value** → **Edit** → **Plaintext** tab.
4. Replace the entire contents with a JSON map of your customer keys:
   ```json
   {
     "customer-one-key-f7k2mXpQ9nRsT4wYv8hLjC3dEuA6bN1": "customer-one",
     "customer-two-key-Q9pXkLhM7vYr2sNcVbEjUuI4aZTwBdF": "customer-two"
   }
   ```
   Each key on the **left** is what you give to that customer; the value on the **right** is the tenant label they appear under in the dashboard. Generate fresh long-random strings for the keys.
5. Click **Save**.

### ReadKeyMapSecret — for the dashboard

Repeat the same steps using the `ReadKeySecretArn` output. The JSON map looks like:
```json
{
  "your-read-key-Hq9xZmRKpVcWnYbT4Lf2sJgN8DAEvUP": "*"
}
```
The value `"*"` denotes admin access (read every tenant's events). To create tenant-scoped read keys, use the tenant id as the value instead of `"*"`.

> The Lambdas re-read both secrets on every invocation, so your keys take effect immediately — no redeploy needed.
