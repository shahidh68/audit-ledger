...
**Example (PowerShell)** — replace the example keys with long random strings before running:

```powershell
cd "C:\Users\AI Data Logger\ai-audit-ledger\infra\cdk"
npx cdk deploy `
  --context tenantKeyMap='{"customer-one-key":"customer-one","customer-two-key":"customer-two"}' `
  --context readKeyMap='{"your-read-key":"*"}'
```

**Rotating keys later (no redeployment needed):** Once deployed, keys live in AWS Secrets Manager. To add, remove, or change a key, update the secret there — the system picks up the change automatically within seconds. You never need to redeploy just to rotate a key.
...
Run the full command with your keys from Step 3 (PowerShell):

```powershell
cd “C:\Users\AI Data Logger\ai-audit-ledger\infra\cdk”
npx cdk deploy `
  --context tenantKeyMap=’{“customer-one-key”:”customer-one”}’ `
  --context readKeyMap=’{“your-read-key”:”*”}’
```
...
**On subsequent deploys** (for code changes, not key rotations): run the same command again. Keys you changed directly in Secrets Manager are unaffected — they are not overwritten on redeploy.

---

## Step 6 — Save what the deployment prints (very important)
...
## Step 7 — Quick “is it working?” checks (optional)
